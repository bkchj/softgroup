...
def get_args():
    ...
    parser.add_argument('--debug_one_batch', action='store_true',
                        help='only run one batch for debug')
    args = parser.parse_args()
    return args


def train(epoch, model, optimizer, scaler, train_loader, cfg, logger, writer, device, debug_one_batch=False):
    model.train()
    ...
    print("[DEBUG] start iterating train_loader", flush=True)

    for i, batch in enumerate(train_loader, start=1):
        if debug_one_batch and i > 1:
            print("[DEBUG] stop after one batch", flush=True)
            break

        data_time.update(time.time() - end)
        cosine_lr_after_step(optimizer, cfg.optimizer.lr, epoch - 1, cfg.step_epoch, cfg.epochs)

        print(f"[DEBUG] entering batch {i}", flush=True)

        with torch.cuda.amp.autocast(enabled=(cfg.fp16 and device.type == 'cuda')):
            print(f"[DEBUG] before model forward batch {i}", flush=True)
            loss, log_vars = model(batch, return_loss=True)
            print(f"[DEBUG] after model forward batch {i}", flush=True)
        ...
        print(f"[DEBUG] before backward batch {i}", flush=True)
        optimizer.zero_grad()
        ...
        scaler.update()
        print(f"[DEBUG] after backward batch {i}", flush=True)
...
def validate(epoch, model, val_loader, cfg, logger, writer):
    ...
    with torch.no_grad():
        ...
        # 关键修复：只有分布式时才调用 collect_results_cpu
        if cfg.dist and world_size > 1:
            results = collect_results_cpu(results, len(val_set))
        else:
            results = results[:len(val_set)]
...
def main():
    ...
    if args.dist and device.type == 'cuda':
    ...
    for epoch in range(start_epoch, cfg.epochs + 1):
        train(epoch, model, optimizer, scaler, train_loader, cfg, logger, writer, device,
              debug_one_batch=args.debug_one_batch)
        if (not args.debug_one_batch) and (not args.skip_validate) and \
