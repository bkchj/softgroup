...
def torch_load_compat(filename):
    ...
    这里显式设为 weights_only=False。
    对旧版 PyTorch，不支持该参数时自动回退。
    """
    try:
...
class S3DISDataset(CustomDataset):
    ...
    def load(self, filename):
        xyz, rgb, semantic_label, instance_label, _, _ = torch_load_compat(filename)
        # subsample data
    ...
    def transform_test(self, xyz, rgb, semantic_label, instance_label):
        ...
        for batch, piece in enumerate([piece_1, piece_2, piece_3, piece_4]):
            ...
            xyz_piece = xyz_middle * self.voxel_cfg.scale
            xyz_piece -= xyz_piece.min(0)
            xyz_list.append(np.concatenate([np.full((xyz_piece.shape[0], 1), batch), xyz_piece], 1))
    ...
    def collate_fn(self, batch):
        ...
        spatial_shape = np.clip(
            (coords.max(0)[0][1:] + 1).numpy(),
            self.voxel_cfg.spatial_shape[0],
            None
        )
