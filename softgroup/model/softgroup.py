...
class SoftGroup(nn.Module):
    ...
    def __init__(self,
        ...
        # geo-instance settings
        self.use_geo_grouping = getattr(grouping_cfg, 'use_geo_grouping', True)
        self.geo_embed_dim = getattr(train_cfg, 'geo_embed_dim', 16) if train_cfg is not None else 16
        self.offset_loss_weight = getattr(train_cfg, 'offset_loss_weight', 0.0 if self.use_geo_grouping else 1.0) if train_cfg is not None else (0.0 if self.use_geo_grouping else 1.0)
        self.embedding_loss_weight = getattr(train_cfg, 'embedding_loss_weight', 1.0) if train_cfg is not None else 1.0
        self.embedding_delta_var = getattr(train_cfg, 'embedding_delta_var', 0.5) if train_cfg is not None else 0.5
        self.embedding_delta_dist = getattr(train_cfg, 'embedding_delta_dist', 1.5) if train_cfg is not None else 1.5
        self.embedding_reg_weight = getattr(train_cfg, 'embedding_reg_weight', 1e-3) if train_cfg is not None else 1e-3
        ...
        self.geo_embedding_linear = MLP(channels, self.geo_embed_dim, norm_fn=norm_fn, num_layers=2)
    ...
    def forward_train(self, batch_idxs, voxel_coords, p2v_map, v2p_map, coords_float, feats,
        ...
        semantic_scores, pt_offsets, geo_embeddings, output_feats = self.forward_backbone(input, v2p_map)

        # point wise losses
        point_wise_loss = self.point_wise_loss(semantic_scores, pt_offsets, geo_embeddings,
                                               semantic_labels, instance_labels, pt_offset_labels)
        losses.update(point_wise_loss)

                # skip expensive geo grouping during training; logging only
        if not self.semantic_only and self.use_geo_grouping:
            losses['num_pos'] = semantic_scores.new_tensor(0.0)
            losses['num_neg'] = semantic_scores.new_tensor(0.0)
        elif not self.semantic_only:
    ...
    def point_wise_loss(self, semantic_scores, pt_offsets, geo_embeddings, semantic_labels,
        ...
        if pos_inds.sum() == 0:
            raw_offset_loss = 0 * pt_offsets.sum()
        else:
            raw_offset_loss = F.l1_loss(
                pt_offsets[pos_inds], pt_offset_labels[pos_inds], reduction='sum') / pos_inds.sum()
        losses['offset_loss'] = raw_offset_loss * self.offset_loss_weight

        if self.use_geo_grouping:
            emb_loss = self.discriminative_embedding_loss(geo_embeddings, instance_labels,
                                                          semantic_labels)
            losses['embedding_loss'] = emb_loss * self.embedding_loss_weight
        return losses

    def discriminative_embedding_loss(self, embeddings, instance_labels, semantic_labels):
        valid_mask = (instance_labels != self.ignore_label) & (semantic_labels != self.ignore_label)
        if valid_mask.sum() == 0:
            return embeddings.sum() * 0

        embeddings = F.normalize(embeddings, dim=1)
        valid_emb = embeddings[valid_mask]
        valid_inst = instance_labels[valid_mask]
        valid_sem = semantic_labels[valid_mask]

        unique_inst = valid_inst.unique(sorted=True)
        if unique_inst.numel() == 0:
            return embeddings.sum() * 0

        means = []
        sem_ids = []
        var_loss = embeddings.new_tensor(0.0)
        n_inst_used = 0
        for inst_id in unique_inst:
            mask = valid_inst == inst_id
            emb_i = valid_emb[mask]
            if emb_i.numel() == 0:
                continue
            mean_i = emb_i.mean(0)
            means.append(mean_i)
            sem_ids.append(valid_sem[mask][0])
            dist_i = torch.norm(emb_i - mean_i.unsqueeze(0), dim=1)
            var_loss = var_loss + torch.mean(F.relu(dist_i - self.embedding_delta_var) ** 2)
            n_inst_used += 1
        if n_inst_used == 0:
            return embeddings.sum() * 0
        var_loss = var_loss / n_inst_used

        means = torch.stack(means, dim=0)
        sem_ids = torch.stack(sem_ids, dim=0)
        reg_loss = torch.mean(torch.norm(means, dim=1))

        dist_loss = embeddings.new_tensor(0.0)
        pair_count = 0
        for cls_id in sem_ids.unique(sorted=True):
            cls_mask = sem_ids == cls_id
            cls_means = means[cls_mask]
            if cls_means.size(0) <= 1:
                continue
            dmat = torch.cdist(cls_means, cls_means, p=2)
            eye = torch.eye(dmat.size(0), device=dmat.device, dtype=torch.bool)
            dist_term = F.relu(self.embedding_delta_dist - dmat[~eye]) ** 2
            if dist_term.numel() > 0:
                dist_loss = dist_loss + dist_term.mean()
                pair_count += 1
        if pair_count > 0:
            dist_loss = dist_loss / pair_count

        return var_loss + dist_loss + self.embedding_reg_weight * reg_loss

    def _edge_prune_by_geo(self, neighbor_inds, start_len, coords, point_scores, point_feats):
        n_points = start_len.size(0)
        if neighbor_inds.numel() == 0 or n_points == 0:
            return neighbor_inds, start_len

        device = coords.device
        lengths = start_len[:, 1].long()
        src = torch.repeat_interleave(torch.arange(n_points, device=device, dtype=torch.long), lengths)
        dst = neighbor_inds.long()
        if src.numel() == 0:
            return neighbor_inds, start_len

        point_feats = F.normalize(point_feats, dim=1)
        feat_sim = (point_feats[src] * point_feats[dst]).sum(1)
        score_pair = torch.minimum(point_scores[src], point_scores[dst])

        feat_thr = getattr(self.grouping_cfg, 'geo_feat_sim_thr', 0.65)
        score_thr = getattr(self.grouping_cfg, 'geo_score_sim_thr', 0.15)
        keep = (feat_sim >= feat_thr) & (score_pair >= score_thr)

        filtered_dst = dst[keep].int().cpu()
        filtered_src = src[keep].cpu()
        if filtered_dst.numel() == 0:
            return filtered_dst, torch.zeros((n_points, 2), dtype=torch.int32)

        kept_counts = torch.bincount(filtered_src, minlength=n_points).int()
        starts = torch.cumsum(kept_counts, dim=0) - kept_counts
        filtered_start_len = torch.stack([starts, kept_counts], dim=1).int()
        return filtered_dst, filtered_start_len

    def _proposal_planarity(self, coords):
        if coords.size(0) < 3:
            return coords.new_tensor(1e6), coords.new_tensor(0.0)
        centered = coords - coords.mean(0, keepdim=True)
        cov = centered.t().mm(centered) / max(coords.size(0) - 1, 1)
        eigvals, eigvecs = torch.linalg.eigh(cov)
        normal = eigvecs[:, 0]
        residual = torch.abs(centered.mm(normal.unsqueeze(1)).squeeze(1)).mean()
        linearity = (eigvals[-1] - eigvals[-2]) / (eigvals[-1] + 1e-6)
        return residual, linearity

    def _filter_and_merge_proposals(self, proposals_idx, proposals_offset, coords_float, semantic_scores,
                                    geo_embeddings):
        if proposals_offset.numel() <= 1:
            return proposals_idx, proposals_offset

        min_points = getattr(self.test_cfg, 'min_npoint', 1)
        planar_min_points = getattr(self.grouping_cfg, 'geo_planar_min_points', min_points)
        planar_res_thr = getattr(self.grouping_cfg, 'geo_planar_residual_thr', 0.08)

        keep_pairs = []
        keep_offsets = [0]
        cur_cluster = 0
        for pid in range(proposals_offset.size(0) - 1):
            s = int(proposals_offset[pid].item())
            e = int(proposals_offset[pid + 1].item())
            if e <= s:
                continue
            pts = proposals_idx[s:e, 1].long()
            if pts.numel() < min_points:
                continue
            coords = coords_float[pts]
            if pts.numel() >= planar_min_points:
                residual, linearity = self._proposal_planarity(coords)
                if residual.item() > planar_res_thr:
                    continue
            chunk = torch.stack([torch.full_like(pts, cur_cluster, dtype=torch.int32), pts.int()], dim=1)
            keep_pairs.append(chunk.cpu())
            keep_offsets.append(keep_offsets[-1] + pts.numel())
            cur_cluster += 1

        if len(keep_pairs) == 0:
            return torch.zeros((0, 2), dtype=torch.int32), torch.zeros((0,), dtype=torch.int32)

        new_idx = torch.cat(keep_pairs, dim=0).int()
        new_offset = torch.tensor(keep_offsets, dtype=torch.int32)
        return new_idx, new_offset
    ...
    def forward_test(self, batch_idxs, voxel_coords, p2v_map, v2p_map, coords_float, feats,
        ...
        semantic_scores, pt_offsets, geo_embeddings, output_feats = self.forward_backbone(
        ...
        if 'semantic' in self.test_cfg.eval_tasks:
            ...
            point_wise_results = self.get_point_wise_results(coords_float, color_feats,
                                                             semantic_preds, semantic_probs,
                                                             pt_offsets, pt_offset_labels,
                                                             v2p_map, lvl_fusion)
        ...
        if not self.semantic_only:
            if 'instance' in self.test_cfg.eval_tasks or 'panoptic' in self.test_cfg.eval_tasks:
                ...
                if self.use_geo_grouping:
                    pred_instances = self.get_instances_geo(
                        scan_ids[0], proposals_idx, proposals_offset, semantic_scores, geo_embeddings,
                        coords_float, v2p_map=v2p_map, lvl_fusion=lvl_fusion)
                else:
    ...
    def forward_backbone(self, input, input_map, x4_split=False, lvl_fusion=False):
        ...
        geo_embeddings = F.normalize(self.geo_embedding_linear(output_feats), dim=1)
        return semantic_scores, pt_offsets, geo_embeddings, output_feats
    ...
    @force_fp32(apply_to=('semantic_scores', 'pt_offsets', 'geo_embeddings'))
    def forward_grouping(self,
                         ...
                         lvl_fusion=False,
                         geo_embeddings=None):
        if not self.use_geo_grouping:
            return self.forward_grouping_original(semantic_scores, pt_offsets, batch_idxs,
                                                  coords_float, grouping_cfg, lvl_fusion)

        proposals_idx_list = []
        proposals_offset_list = []
        batch_size = batch_idxs.max() + 1
        semantic_prob = semantic_scores.softmax(dim=-1)
        ...
        for class_id in range(self.semantic_classes):
            if class_id in self.grouping_cfg.ignore_classes:
                continue
            scores = semantic_prob[:, class_id].contiguous()
            ...
            scores_ = scores[object_idxs]
            embeds_ = geo_embeddings[object_idxs] if geo_embeddings is not None else semantic_prob[object_idxs]
            ...
            neighbor_inds, start_len = self._edge_prune_by_geo(neighbor_inds, start_len, coords_,
                                                               scores_, embeds_)
            proposals_idx, proposals_offset = bfs_cluster(class_numpoint_mean, neighbor_inds,
                                                          start_len, npoint_thr, class_id)

            proposals_idx[:, 1] = object_idxs[proposals_idx[:, 1].long()].int()
            proposals_idx, proposals_offset = self._filter_and_merge_proposals(
                proposals_idx, proposals_offset, coords_float, semantic_prob, geo_embeddings)

            if len(proposals_offset_list) > 0 and proposals_offset.numel() > 0:
            ...
            if proposals_idx.size(0) > 0 and proposals_offset.numel() > 0:
    ...
    def forward_grouping_original(self,
                                  semantic_scores,
                                  pt_offsets,
                                  batch_idxs,
                                  coords_float,
    ...
    @force_fp32(apply_to=('semantic_preds', 'semantic_probs', 'offset_preds'))
    def get_point_wise_results(self, coords_float, color_feats, semantic_preds, semantic_probs,
                               offset_preds, offset_labels, v2p_map, lvl_fusion):
        if lvl_fusion:
            semantic_preds = semantic_preds[v2p_map.long()]
            semantic_probs = semantic_probs[v2p_map.long()]
        ...
        return dict(
            ...
            semantic_probs=semantic_probs.cpu().numpy(),
    ...
    def get_instances(self,
        ...
        for i in range(self.instance_classes):
            if i in self.sem2ins_classes:
                ...
                mask_inds_cpu = mask_inds.detach().cpu()
                cur_proposals_idx = proposals_idx[mask_inds_cpu].long().to(mask_pred.device)
    ...
    @force_fp32(apply_to=('semantic_scores', 'geo_embeddings'))
    def get_instances_geo(self,
                          scan_id,
                          proposals_idx,
                          proposals_offset,
                          semantic_scores,
                          ...
                          v2p_map=None,
                          lvl_fusion=False):
        if proposals_idx.size(0) == 0 or proposals_offset.numel() <= 1:
            return []

        semantic_prob = semantic_scores.softmax(1)
        instances = []
        min_npoint = getattr(self.test_cfg, 'min_npoint', 1)
        feat_thr = getattr(self.grouping_cfg, 'geo_feat_sim_thr', 0.65)
        planar_res_thr = getattr(self.grouping_cfg, 'geo_planar_residual_thr', 0.08)
        ...
        for pid in range(proposals_offset.size(0) - 1):
            ...
            if e <= s:
                continue
            pts = proposals_idx[s:e, 1].long()
            if pts.numel() < min_npoint:
                continue

            sem_mean = semantic_prob[pts].mean(0)
            label_id = int(torch.argmax(sem_mean).item()) + 1
            sem_conf = float(sem_mean[label_id - 1].item())

            emb = F.normalize(geo_embeddings[pts], dim=1)
            emb_center = F.normalize(emb.mean(0, keepdim=True), dim=1)
            emb_cohesion = float(((emb * emb_center).sum(1).mean().clamp(-1, 1) + 1) * 0.5)

            residual, linearity = self._proposal_planarity(coords_float[pts])
            planar_conf = max(0.0, 1.0 - float(residual.item()) / max(planar_res_thr, 1e-6))
            score = sem_conf * emb_cohesion * planar_conf

            mask_pred = torch.zeros((semantic_scores.size(0),), dtype=torch.int, device=semantic_scores.device)
            mask_pred[pts] = 1
            if lvl_fusion:
                mask_pred = mask_pred[v2p_map.long()]
            ...
            pred = {}
            pred['scan_id'] = scan_id
            pred['label_id'] = label_id
            pred['conf'] = float(score)
